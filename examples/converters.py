...
@bot.command()
async def userinfo(ctx: commands.Context, user: selfcord.User):
    # In the command signature above, you can see that the `user`
    # parameter is typehinted to `selfcord.User`. This means that
    # during command invocation we will attempt to convert
    # the value passed as `user` to a `selfcord.User` instance.
    # The documentation notes what can be converted, in the case of `selfcord.User`
    ...
    # If the conversion is successful, we will have a `selfcord.User` instance
...
class ChannelOrMemberConverter(commands.Converter):
    async def convert(self, ctx: commands.Context, argument: str):
        ...
        # `selfcord.Member` or `selfcord.TextChannel` instance from the
...
@bot.command()
async def ignore(ctx: commands.Context, target: typing.Union[selfcord.Member, selfcord.TextChannel]):
    ...
    # So, it will attempt to convert whatever is passed to `target` to a `selfcord.Member` instance.
    # If that fails, it will attempt to convert it to a `selfcord.TextChannel` instance.
    ...
    if isinstance(target, selfcord.Member):
        await ctx.send(f'Member found: {target.mention}, adding them to the ignore list.')
    elif isinstance(target, selfcord.TextChannel):  # this could be an `else` but for completeness' sake.
