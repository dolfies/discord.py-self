class Sidebar {
  constructor(element) {
    ...
    this.element.addEventListener("click", (e) => {
      ...
      if (
        e.target.tagName == "A" &&
        this.element.classList.contains("sidebar-toggle")
      ) {
  ...
  createCollapsableSections() {
    ...
    let allReferences = toc.querySelectorAll(
      'a.reference.internal:not([href="#"])'
    );
  ...
  resize() {
    ...
    this.element.style.height = `calc(100vh - 1em - ${
      rect.top + document.body.offsetTop
    }px)`;
  }
  ...
  setActiveLink(section) {
    ...
    if (section) {
      ...
      if (this.activeLink) {
        ...
        let heading =
          headingChildren.previousElementSibling.previousElementSibling;
